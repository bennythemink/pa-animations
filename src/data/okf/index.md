---
okf_version: '0.1'
---

# Pixel Agency Knowledge Bundle

An [Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) (v0.1) bundle
describing Pixel Agency's services, case studies, and published insights, intended for consumption by internal tooling
and AI systems (including a future WebMCP server for this site).

# Sections

* [Company](/company/index.md) - Who Pixel Agency is and how to contact us.
* [Services](/services/index.md) - The services Pixel Agency offers.
* [Products](/products/index.md) - Pixel Agency's own products.
* [Case Studies](/case-studies/index.md) - Published client case studies.
* [Insights](/insights/index.md) - Published articles on discovery, delivery, and AI.
